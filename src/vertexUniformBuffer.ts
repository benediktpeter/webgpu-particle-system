export class VertexUniformBuffer {
    ...
    private readonly USE_PIXEL_SIZES_OFFSET: number = this.ROTATION_ENABLED_OFFSET + 4;
    ...
    public setHeight(height: number, inPixels: boolean = false): void {
        let halfHeight = (height * 0.5) * this.PARTICLE_SIZE_FACTOR;
        if(inPixels) {
            halfHeight = height / this._canvasHeight;
        }
        this._device.queue.writeBuffer(this._uniformBuffer, this.HALFHEIGHT_OFFSET, Float32Array.of(halfHeight));
    }

    public setWidth(width: number, inPixels: boolean = false) : void {
        let halfWidth = (width * 0.5) * this.PARTICLE_SIZE_FACTOR;
        if(inPixels) {
            halfWidth = width / this._canvasWidth;
        }
    ...
    public setUsePixelSizes(usePixelSizes: boolean) {
        this._device.queue.writeBuffer(this._uniformBuffer, this.USE_PIXEL_SIZES_OFFSET, Uint32Array.of(usePixelSizes ? 1 : 0))
    }
